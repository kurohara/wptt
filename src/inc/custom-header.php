...
/**
 * Custom Header functionality for wptt
 *
 * @package WordPress
...
function custom_header_setup() {
	...
	/**
	 * Filter wptt custom-header support arguments.
	 *
	 * @since wptt 1.0
	 *
	 * @param array $args {
	 ...
	 * }
	 */
	add_theme_support( 'custom-header', ns_apply_filters( 'custom_header_args', array(
...
if ( ! ns_function_exists( 'header_style' ) ) :
/**
...
function header_style() {
	...
	?>
	<style type="text/css" id="<?php echo ns_('-header-css');?>">
	<?php
...
function header_background_color_css() {
	...
	wp_add_inline_style( ns_('-style'), sprintf( $css, $header_background_color ) );
}
add_ns_action( 'wp_enqueue_scripts', 'header_background_color_css', 11 );
 ...
 * @since wptt 1.0
 */
function sidebar_text_color_css() {
	$color_scheme       = get_color_scheme();
	...
	// If we get this far, we have custom styles. Let's do this.
	$sidebar_link_color_rgb     = hex2rgb( $sidebar_link_color );
	...
	wp_add_inline_style( ns_('-style'), sprintf( $css, $sidebar_link_color, $sidebar_text_color, $sidebar_border_color, $sidebar_border_focus_color ) );
}
add_ns_action( 'wp_enqueue_scripts', 'sidebar_text_color_css', 11 );
