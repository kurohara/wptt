...
function customize_register( $wp_customize ) {
	...
	$wp_customize->add_setting( 'color_scheme', array(
		'default'           => 'default',
		'sanitize_callback' => ns_name('sanitize_color_scheme'),
	...
	$wp_customize->get_section( 'header_image' )->description = __( 'Applied to the header on small screens and the sidebar on wide screens.', ns_('') );
}
add_ns_action( 'customize_register', 'customize_register', 11 );

/**
 * Register color schemes for wptt
 *
 * Can be filtered with {@see 'color_schemes'}.
 *
...
function get_color_schemes() {
	return ns_apply_filters( 'color_schemes', array(
		'default' => array(
			'label'  => __( 'Default', ns_('') ),
			'colors' => array(
				'#f1f1f1',
				'#ffffff',
				'#ffffff',
...
/**
 * Get the current wptt color scheme.
 *
 * @since wptt 1.0
 *
...
endif; // get_color_scheme

if ( ! ns_function_exists( 'get_color_scheme_choices' ) ) :
/**
 * Returns an array of color scheme choices registered for wptt.
 *
 * @since wptt 1.0
 *
...
endif; // get_color_scheme_choices

if ( ! ns_function_exists( 'sanitize_color_scheme' ) ) :
/**
...
function sanitize_color_scheme( $value ) {
	$color_schemes = get_color_scheme_choices();
...
function color_scheme_css() {
	...
	$color_textcolor_rgb         = hex2rgb( $color_scheme[3] );
	$color_sidebar_textcolor_rgb = hex2rgb( $color_scheme[4] );
	$colors = array(
	...
	$color_scheme_css = get_color_scheme_css( $colors );

	wp_add_inline_style( ns_('-style'), $color_scheme_css );
}
add_ns_action( 'wp_enqueue_scripts', 'color_scheme_css' );
...
function customize_control_js() {
	...
	wp_localize_script( 'color-scheme-control', 'colorScheme', get_color_schemes() );
    wp_localize_script( 'color-scheme-control', 'themeName', ns_(''));
}
add_ns_action( 'customize_controls_enqueue_scripts', 'customize_control_js' );
...
function customize_preview_js() {
	wp_enqueue_script( ns_('-customize-preview'), get_template_directory_uri() . '/js/customize-preview.js', array( 'customize-preview' ), '20141216', true );
    wp_localize_script( ns_('-customize-preview'), 'themeName', ns_(''));
}
add_ns_action( 'customize_preview_init', 'customize_preview_js' );
...
function color_scheme_css_template() {
	...
	?>
	<script type="text/html" id="tmpl-<?php echo ns_('-color-scheme');?>">
		<?php echo get_color_scheme_css( $colors ); ?>
	</script>
	<?php
}
